...
export declare type ExecutionError = ExpressionError | WorkflowActivationError | WorkflowOperationError | NodeOperationError | NodeApiError;
...
declare type Override<A extends object, B extends object> = Omit<A, keyof B> & B;
...
export declare type CredentialInformation = string | number | boolean | IDataObject | IDataObject[];
...
export declare type ResourceLocatorModes = 'id' | 'url' | 'list' | string;
...
export declare type NodeParameterValueType = NodeParameterValue | INodeParameters | INodeParameterResourceLocator | NodeParameterValue[] | INodeParameters[] | INodeParameterResourceLocator[];
...
export declare type INodePropertyValueExtractor = INodePropertyValueExtractorRegex;
...
export declare type PostReceiveAction = ((this: IExecuteSingleFunctions, items: INodeExecutionData[], response: IN8nHttpFullResponse) => Promise<INodeExecutionData[]>) | IPostReceiveBinaryData | IPostReceiveLimit | IPostReceiveRootProperty | IPostReceiveSet | IPostReceiveSetKeyValue | IPostReceiveSort;
...
export declare type DocumentationLink = {
    url: string;
};
...
export declare type PublicInstalledPackage = {
    packageName: string;
...
export declare type PublicInstalledNode = {
    name: string;
    type: string;
